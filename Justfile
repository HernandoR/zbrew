...
# Sibling of the directory `zb` is installed into, because that is where `man`
# looks: given `~/.local/bin` on PATH it searches `~/.local/share/man` and
# `~/.local/man` without being told to.
ZBREW_MAN := env('ZBREW_MAN', parent_directory(ZBREW_BIN) / 'share' / 'man')
...
install: build
    ...
    just _install_man "$ZBREW_BIN/zb"

    "$ZBREW_BIN/zb" init

# Generated by the zb that was just installed, so the manual always describes
# that binary. Never fatal: a missing man page does not stop zb from working.
[private]
[script]
_install_man zb:
    # Drop a previous install's pages first, so a subcommand that has since been
    # renamed leaves none behind. Only zbrew's own: the man1 directory is shared
    # with everything else installed alongside zb.
    rm -f -- "$ZBREW_MAN/man1"/zb.1 "$ZBREW_MAN/man1"/zb-*.1
    if ! "{{zb}}" man --output-dir "$ZBREW_MAN/man1"; then
        echo "Warning: could not install man pages to $ZBREW_MAN/man1" >&2
        exit 0
    fi
    echo "Installed man pages to $ZBREW_MAN/man1"
...
uninstall:
    ...
    echo -e  "\t$ZBREW_DIR"
    echo -e  "\t$ZBREW_ROOT"
    echo -e  "\t$ZBREW_MAN/man1/zb.1 and its per-subcommand pages"
    ...
    # Only zbrew's own pages: the man1 directory is shared with everything else
    # installed alongside zb.
    rm -f -- "$ZBREW_MAN/man1"/zb.1 "$ZBREW_MAN/man1"/zb-*.1
