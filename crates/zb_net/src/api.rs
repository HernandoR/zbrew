...
use zb_core::checksum::verify_sha256_bytes;
...
impl ApiClient {
    ...
    fn build_client(base_urls: Vec<String>, cask_base_urls: Vec<String>) -> Self {
        ...
        let client = reqwest::Client::builder()
            ...
            .use_preconfigured_tls((*crate::tls::shared_tls_config()).clone())
            .build()
    ...
    // Point the tap/cask endpoints at a mock server. Previously `#[cfg(test)]`,
    // when the installer tests that need them lived in the same crate; they now
    // live in `zb_installer`, so they are reached through the `test-util`
    // feature that crate's dev-dependency turns on.
    ...
    async fn get_tap_formula(
        &self,
        spec: &crate::tap_formula::TapFormulaRef,
