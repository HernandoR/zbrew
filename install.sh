...
# Sibling of the directory the binaries go into, because that is where `man`
# looks: given `~/.local/bin` on PATH it searches `~/.local/share/man` and
# `~/.local/man` without being told to.
: "${ZBREW_MAN:="$(dirname -- "$ZBREW_BIN")/share/man"}"
ORIGINAL_PATH="$PATH"
PREVIOUS_ZB_VERSION=""
...
# The pages are generated by the binary that was just installed, so they always
# describe that binary's commands. Never fatal: a missing man page does not stop
# zb from working.
install_man() {
    local man1_dir="$ZBREW_MAN/man1"

    if ! mkdir -p "$man1_dir" 2>/dev/null; then
        warn "Could not create $man1_dir. Skipping man pages; generate them later with 'zb man --output-dir DIR'."
        return 0
    fi

    # Drop the pages of a previous install first, so a subcommand that has since
    # been renamed does not leave its page behind. Only zbrew's own pages: the
    # man1 directory is shared with everything else installed alongside zb.
    rm -f "$man1_dir"/zb.1 "$man1_dir"/zb-*.1 2>/dev/null || true

    if ! "$ZBREW_BIN/zb" man --output-dir "$man1_dir"; then
        warn "Could not install man pages to $man1_dir. Generate them later with 'zb man --output-dir DIR'."
        return 0
    fi

    completed "Installed ${ORANGE}man pages${NC} to $man1_dir"
}
