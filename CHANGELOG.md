...
### Added
- `zb info` answers for a formula you have not installed, the way `brew info` does, and says much more about one you have. It now prints the description, homepage and license, the keg's path with its file count and size on disk, the runtime and build dependencies, and why a keg-only formula is not linked. Asking about a package before installing it was the gap that mattered: the old `zb info` replied "Formula 'jq' is not installed." and stopped, which is also why the Homebrew-compatibility workflow, whose whole job is running `zb info` over twenty common formulas, had been passing without fetching anything. Metadata is best-effort for an installed package -- a keg whose formula the API no longer serves is still described from what is on disk ([#26](https://github.com/HernandoR/zbrew/issues/26), [upstream #162](https://github.com/lucasgelfond/zerobrew/issues/162))
- A name read out of `zb list` works when typed back into `zb info`, `zb uninstall` or `zb upgrade`, including a cask. Casks are stored under a `cask:` install name so they cannot collide with a formula of the same name, but that prefix is an internal key and the listing now prints a cask as `docker`; those three commands resolve a bare name against what is installed, so `docker` finds the cask when no formula of that name is installed. A formula always wins over a cask of the same name, and `zb install` is deliberately unchanged -- there a bare name still means a formula and `cask:` is how you ask for a cask. `zb upgrade` says a cask cannot be upgraded and carries on with the rest of the batch, rather than aborting on it: the formula API has nothing to say about a cask, so asking it would end the command before the names after it were reached ([#26](https://github.com/HernandoR/zbrew/issues/26))
- `zb list --json` writes the installed packages as JSON -- name, kind, version, store key, install timestamp and install reason -- so a script no longer has to parse coloured output. The human listing now separates formulae from casks under headings, and a cask is printed under its own token rather than the internal `cask:` key ([#26](https://github.com/HernandoR/zbrew/issues/26))
