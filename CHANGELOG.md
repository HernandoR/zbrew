...
### Added
- Homebrew's mirror environment variables are read, so a shell profile that already points `brew` at a mirror points `zb` at it too. `HOMEBREW_API_DOMAIN` moves formula and cask metadata, `HOMEBREW_BOTTLE_DOMAIN` moves bottles and their manifests, and `HOMEBREW_ARTIFACT_DOMAIN` moves every download at once — prefixing an ordinary URL whole, replacing the registry host of a bottle URL, and not repeating a `/v2` segment the value already carries. Each of them is a *preference*, not a redirect: the mirror is tried first and the default is the fallback, so a mirror that is down, half-synced or missing one file does not make a package look deleted. `HOMEBREW_ARTIFACT_DOMAIN_NO_FALLBACK` turns that off for anyone who needs requests to never leave the mirror. A value that is not an http(s) URL, or that carries credentials, is reported and ignored rather than used. zbrew's own `ZBREW_API_URL` still wins over `HOMEBREW_API_DOMAIN` and still gets no fallback, and `HOMEBREW_BOTTLE_MIRRORS`, which zbrew had before any of this, keeps working ([#31](https://github.com/HernandoR/zbrew/issues/31), [#62](https://github.com/HernandoR/zbrew/issues/62), [upstream #341](https://github.com/lucasgelfond/zerobrew/issues/341), [upstream #392](https://github.com/lucasgelfond/zerobrew/pull/392))
