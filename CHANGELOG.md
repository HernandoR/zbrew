...
### Added
- `zb bundle check` reports whether a Brewfile is satisfied and says so in its exit code, matching `brew bundle check`. It prints the entries that are missing and exits non-zero, or confirms and exits 0, so it can gate an `.envrc` or a CI step without installing anything. A package installed only as a dependency of another counts as satisfied; a keg `zb run` left behind does not, because `zb gc` deletes those. An empty Brewfile satisfies `check` -- nothing is required, so nothing is missing, matching `brew bundle check` -- while still being an error for `bundle install`, which was asked to do something. Entries are deduplicated by their normalised name, so `cask "docker"` and `homebrew/cask/docker` are one package rather than two ([#34](https://github.com/HernandoR/zbrew/issues/34), [upstream #376](https://github.com/lucasgelfond/zerobrew/issues/376))
...
### Changed
- `zb bundle install` skips entries that are already installed instead of reinstalling the whole manifest. Every invocation used to re-plan, re-unpack and re-link each package -- the bottles came from the blob cache, so nothing was re-downloaded, but the work was redone -- which is what made `zb bundle` unusable in an `.envrc` that runs on every directory change. A satisfied Brewfile now issues no requests at all and returns immediately. `brew bundle install` behaves the same way; upgrading remains `zb upgrade`'s job ([#34](https://github.com/HernandoR/zbrew/issues/34))
