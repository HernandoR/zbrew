...
### Fixed
- `zb migrate` no longer skips a formula homebrew-core has dropped. Migration read every formula from the live JSON API, so a package the user still has installed but the API no longer serves -- `fasd` is the reported case -- was reported as missing and left behind, which is the opposite of what migration is for. Homebrew keeps the formula it installed at `<prefix>/Cellar/<name>/<version>/.brew/<name>.rb`, and that copy is now read when the API has nothing: it names the same bottle, at the same registry, that Homebrew itself would have poured. The fallback runs inside the dependency walk, so a recovered formula's own dependencies are still resolved normally, and it is reached only on a genuine 404 -- every name `zb migrate` passes in came from `brew leaves` and therefore always has a local copy, so falling back on a 5xx would let one API outage migrate the whole machine from stale metadata and call it success. A name Homebrew never installed is still an error rather than a phantom package. Two things a recovered formula used to lose are now read from the `.rb` as well: `keg_only`, without which a keg-only package such as `openssl@3` or `libpq` would be symlinked into the prefix, and the `@`/`+` mapping Homebrew applies to publish `openssl@1.1` at `homebrew/core/openssl/1.1` -- `@`-versioned formulas being precisely the ones homebrew-core drops, every recovered bottle URL would otherwise have 404ed. Where several versions of a formula are installed side by side, the newest is used, ordered by number rather than by string so `1.10.0` beats `1.9.0` ([#35](https://github.com/HernandoR/zbrew/issues/35), [upstream #379](https://github.com/lucasgelfond/zerobrew/issues/379))
