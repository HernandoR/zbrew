...
### Added
- Homebrew's mirror environment variables are read, so a shell profile that already points `brew` at a mirror points `zb` at it too. `HOMEBREW_API_DOMAIN` moves formula and cask metadata, and `HOMEBREW_ARTIFACT_DOMAIN` moves every download at once — prefixing an ordinary URL whole, replacing the registry host of a bottle URL, and not repeating a `/v2` segment the value already carries. Both are a *preference*, not a redirect: the mirror is tried first and the default is the fallback, so a mirror that is down, half-synced or missing one file does not make a package look deleted. Metadata requests honour that order strictly; bottle downloads race the candidates on a stagger, so the default domain is still contacted even when the mirror answers — `HOMEBREW_ARTIFACT_DOMAIN_NO_FALLBACK` leaves a single candidate and so no race, for anyone who needs requests to never leave the mirror. A value that is not an http(s) URL, or that carries credentials, is reported and ignored rather than used. Registry tokens are now cached per host as well as per scope, because a token minted by a mirror is not valid upstream and keying on the scope alone let a mirror's token be sent to ghcr.io and break the fallback. zbrew's own `ZBREW_API_URL` still wins over `HOMEBREW_API_DOMAIN` and still gets no fallback, and `HOMEBREW_BOTTLE_MIRRORS`, which zbrew had before any of this, keeps working. `HOMEBREW_BOTTLE_DOMAIN` is deliberately **not** implemented: Homebrew serves flat `name--version.tag.bottle.tar.gz` files from any bottle domain that is not GitHub Packages, zbrew cannot yet build that filename, and rewriting only the host would produce a URL every such mirror answers with a 404 — silently, since the upstream URL is always appended as a fallback; tracked separately in [#120](https://github.com/HernandoR/zbrew/issues/120) ([#31](https://github.com/HernandoR/zbrew/issues/31), [#62](https://github.com/HernandoR/zbrew/issues/62), [upstream #341](https://github.com/lucasgelfond/zerobrew/issues/341), [upstream #392](https://github.com/lucasgelfond/zerobrew/pull/392))
