...
### Fixed
- `zb migrate` no longer skips a formula homebrew-core has dropped. Migration read every formula from the live JSON API, so a package the user still has installed but the API no longer serves -- `fasd` is the reported case -- was reported as missing and left behind, which is the opposite of what migration is for. Homebrew keeps the formula it installed at `<prefix>/Cellar/<name>/<version>/.brew/<name>.rb`, and that copy is now read when the API has nothing: it names the same bottle, at the same registry, that Homebrew itself would have poured. The fallback runs inside the dependency walk, so a recovered formula's own dependencies are still resolved normally, and it is consulted only after the API has failed -- a name Homebrew never installed is still an error rather than a phantom package. Where several versions of a formula are installed side by side, the newest is used, ordered by number rather than by string so `1.10.0` beats `1.9.0` ([#35](https://github.com/HernandoR/zbrew/issues/35), [upstream #379](https://github.com/lucasgelfond/zerobrew/issues/379))
